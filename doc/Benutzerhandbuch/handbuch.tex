...
Auf diesem Bildschirm wird entschieden, ob ein neues Spiel angefangen oder eine begonnene Partie weitergeführt werden soll. 
\begin{itemize}
\item New Game: leitet direkt zur Schwierigkeitsauswahl weiter.
\item Continue: führt zum Ladebildschirm für entweder Single- oder Multiplayer, je nach vorheriger Auswahl.
\item Back: kann zurück ins Hauptmenü gelangt werden.
\end {itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{DasSpiel/Menu/ladesingle.png}
\includegraphics[width=0.5\linewidth]{DasSpiel/Menu/lademulti.png}
\end{figure} 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{DasSpiel/Menu/pause.png}
\end{figure} 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{DasSpiel/Menu/serverjoin.png}
\end{figure} 
...
Treibstoff wird benötigt um von einem Stern zum anderen springen zu können. Ein Sprung benötigt eine Treibstoff Einheit. Falls Ihr Treibstoff durch ein zufälliges Planet-Event reduziert oder durch zu viele Sprünge auf Null fällt, so ist das Spiel vorbei und Sie treiben ohne Treibstoff in der leere des Weltraums herum. Falls Sie sich dann nicht zufällig an einem Planeten mit einem \nameref{Shop} aufhalten und sich Treibstoff kaufen können, so gilt die Partie als vorbei.
\begin{figure}[H]
\centering
...
\end{figure}
Ist der Balken komplett Rot so ist der Antrieb komplett aufgeladen und das Schiff ist zum Sprung bereit. Um einen Sprung durchzuführen, muss sich außerhalb eines Kampfes ein Crewmitglied im Antrieb und ein weiteres im Cockpit befinden. Die Funktion der \nameref{sec:Systeme} werden im weiteren Verlauf erklärt.\\
...
Die hier zusehenden Nummerierungen, ist in der Reihenfolge gewählt wie auch die Systeme in \nameref{sec:Energiemanagement} aufgelistet sind, falls also die Funktion der unterschiedlichen Räume bekannt sind und Sie nur wissen wollen wie die Energieverteilung funktioniert, so können Sie zu dem angegebenen Kapitel springen.
...
Auf dem Inventar sieht man unten links (1) die Anzahl an Fuel und die Anzahl der Missiles. Unten Rechts (2) ist der Knopf zum schließen des Fensters. Oben links (3) sieht man seine Crewmitglieder aufgelistet (siehe \nameref{sec:Crew}). Oben rechts sind alle \nameref{sec:Waffen} aufgelistet. 
...
Die normalen Planeten sind eine Zufällig generierte Anzahl an Planeten, auf denen Zufällig eins aus 6 \nameref{sec:Events} auftritt.
...
\subparagraph{Gegnerisches Schiff} Hier kann man ein zufälliges Geschenk erhalten. Hier wird ein gegnerisches Schiff angezeigt, welches in einem \nameref{sec:Kampf}. Die normalen Gegner haben immer die gleichen Stats und Waffen, welche der Grundausstattung der Schiffe entspricht, aus denen der Spieler auch auswählen kann. Beim Gewinnen bekommt der Spieler eine zufällige Auswahl an Sachen geschenkt. 
...
\subparagraph{Shop}
 Beim \nameref{Shop} ist es dem Spieler möglich, Ausrüstung zu kaufen oder zu verkaufen.
...
Der Kampf funktioniert, wie in \nameref{Erklärung} erklärt. Die Gegner haben feste Stats, welche bei jedem Gegner dieses Typs gleich sind. Sie haben immer die gleiche Ausrüstung, welche der der Startschiffe des Spielers entsprechen. 
